...
async function api(action, payload={}){
  if (!SCRIPT_URL || SCRIPT_URL.includes("PEGA_AQUI")) {
    throw new Error("Falta configurar SCRIPT_URL en app.js");
  }
  ...
  // Si el service worker devuelve algo raro, protegemos aquí también
  let data;
  try { data = await res.json(); }
  catch { throw new Error("Respuesta no válida del servidor (¿offline?)"); }
...
async function moneRegister(){
  ...
  if(role === "admin" && !adminPass){
    alert("Para crear admin necesitas la contraseña.");
    return;
  }
...
window.moneClearNotif = moneClearNotifications;
